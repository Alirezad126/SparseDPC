...
nsteps_test=300
# generate reference
np_refs = psl.signals.step(nsteps_test+1, 1, min=xmin, max=xmax, randsteps=20, rng=np.random.default_rng(20))
...
search_space = {"n_steps": tune.grid_search([10]), "n_iter": tune.grid_search([1]),
               "max_degree": tune.grid_search([2]), "max_freq": tune.grid_search([5]),
               "train": tune.grid_search([train_func]),
               "lambda": tune.grid_search([1e-7])}
...
results.get_dataframe().to_csv("csv/tt-id.csv")
...
results.get_dataframe().to_csv("csv/tt-weighted-id.csv")
...
results.get_dataframe().to_csv("csv/tt-lp-id.csv")
...
results.get_dataframe().to_csv("csv/tt-curr-id.csv")
...
with open('csv/tt_data-id.pyc', 'ab') as outfile:
