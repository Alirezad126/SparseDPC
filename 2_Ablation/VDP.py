...
search_space = {"n_steps": tune.grid_search([2, 5, 10, 20]), "n_iter": tune.grid_search([1]),
               "max_freq": tune.grid_search([0, 2, 5]), "max_degree": tune.grid_search([0, 1, 2]),
               "train": tune.grid_search([train_func]),
               "lambda": tune.grid_search([1e-7, 1e-3, 1e-1, 1e1])}
...
results.get_dataframe().to_csv("csv/vdp.csv")
...
results.get_dataframe().to_csv("csv/vdp-weighted.csv")
...
results.get_dataframe().to_csv("csv/vdp-lp.csv")


# In[16]:


search_space = {"n_steps": tune.grid_search([2,]), "n_iter": tune.grid_search([1, 2, 3, 4, 5]),
               "max_freq": tune.grid_search([config["max_freq"]]), "max_degree": tune.grid_search([config["max_degree"]]),
               "lambda": tune.grid_search([config["lambda"]]),
               "train": tune.grid_search([train_func]),
               }
...
results.get_dataframe().to_csv("csv/vdp-cirr.csv")
...
names = ["reg", "weighted", "lp", "cirriculum"]
...
with open('csv/vdp_data_wb.pyc', 'ab') as outfile:
